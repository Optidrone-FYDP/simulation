...
model = DroneMovementModel(
    input_dim=4, hidden_dim=HIDDEN_SIZE, output_dim=3, num_layers=NUM_LAYERS
).to(DEVICE)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch % 100 == 1:
        print(
            f"Epoch {epoch}/{EPOCHS} - Train Loss: {avg_train_loss:.4f}, Test Loss: {avg_test_loss:.4f}"
        )
